...
The idea is to find, for each critical workload in the disaster zone or region, 
...
# machine_type, tags (to keep track of criticality).
...
def get_critical_workloads_in_region(region):
    ...
    and tag columns"""
    return df.loc[
        (df["region"] == region) & (df["tags"].map(lambda x: "critical" in x))
    ]
...
def get_non_critical_workloads_in_region(region):
    ...
    return df.loc[
        (df["region"] == region) & (df["tags"].map(lambda x: "non-critical" in x))
    ]
...
def get_all_non_critical_workloads_regional_disaster(disaster_region):
    ...
    return df.loc[
        (df["tags"].map(lambda x: "non-critical" in x))
        & (df["region"] != disaster_region)
    ]
...
def get_all_non_critical_workloads_zonal_disaster(disaster_zone):
    ...
    Non-critical workloads are identified if they contain the non-critical tag"""
    return df.loc[
        (df["tags"].map(lambda x: "non-critical" in x)) & (df["zone"] != disaster_zone)
    ]
...
def get_non_critical_workloads_in_zone(zone):
    ...
    are identified if they contain the non-critical tag"""
    return df.loc[
        (df["zone"] == zone) & (df["tags"].map(lambda x: "non-critical" in x))
    ]


# BUG: Re-uses noncritical workloads that have already been mapped
def possible_workload_mapping(
    disaster_critical_workload, recovery_non_critical_workload
):
...
def unmappable_critical_workloads(
    disaster_critical_workload, recovery_non_critical_workload
):
    """Get all the critical workloads in the disaster region or zone that do not
    have a match in the recovery region or zone"""
    return disaster_critical_workload.merge(
        recovery_non_critical_workload, on="instance_type", how="outer", indicator=True
    ).loc[lambda x: x["_merge"] == "left_only"]


# Could be carried out in rounds (best efforts) as long as critical workloads
# remain in the disaster zone
def region_to_region_recovery(disaster_region, recovery_region):
    ...
    # machine type
    print(
        "Mapping critical workloads in the disaster region to non-critical workloads in the recovery region"
    )
    mapped_workloads = possible_workload_mapping(
        disaster_region_critical_workloads, recovery_region_non_critical_workloads
    )
    print(mapped_workloads.head())

    print(
        "Warning the following critical workloads in the disaster region do not have a match in the recovery region"
    )
...
def region_to_many_recovery(disaster_region):
    """Identify and match workloads in the disaster region to all non-critical
    workloads"""
    disaster_region_critical_workloads = get_critical_workloads_in_region(
        disaster_region
    )
    ...
    print("\n")
    print(
        "All possible mappings of critical workloads in the disaster region to non-critical workloads in recovery regions"
    )
    mapped_workloads = possible_workload_mapping(
    ...
    print("\n")
    print(
        "WARNING: The following critical workloads in the disaster region do not have a match in any recovery region"
    )
    unmapped_workloads = unmappable_critical_workloads(
        disaster_region_critical_workloads, all_non_critical_workloads
    )
    ...
    for critical_workload in mapped_workloads["project_x"].unique():
        ...
        first_match = mapped_workloads.loc[
            mapped_workloads["project_x"] == critical_workload
        ].iloc[0]
        ...
        mapped_workloads = mapped_workloads[
            mapped_workloads["project_x"] != critical_workload
        ]
        ...
        mapped_workloads = mapped_workloads[
            mapped_workloads["project_y"] != first_match["project_y"]
        ]

    print("\n")
    print(
        "The following critical workloads in the disaster region have been mapped to non-critical workloads in recovery regions"
    )
    print(new_mapped_workloads.head())


# TODO: A better way to do this is as a constraint satisfaction problem using a
